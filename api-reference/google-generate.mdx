---
title: "Google Gemini Generate"
---

<Info>
    **官方文档**
    - [Google Gemini Generating API](https://ai.google.dev/api/generate-content)
</Info>
...
```bash
curl "https://api.llmhub.com.cn/v1beta/models/gemini-2.0-flash:generateContent?key=$LLM_Hub_API_KEY" \
...
```bash
curl "https://api.llmhub.com.cn/v1beta/models/gemini-2.0-flash:streamGenerateContent?alt=sse&key=$LLM_Hub_API_KEY" \
    -H 'Content-Type: application/json' \
    --no-buffer \
    -d '{
      "contents": [{
...
#### 生成内容
```
POST https://api.llmhub.com.cn/v1beta/{model=models/*}:generateContent
```

#### 流式生成内容
```
POST https://api.llmhub.com.cn/v1beta/{model=models/*}:streamGenerateContent
```
...
其中 `$LLM_Hub_API_KEY` 是您的 Google AI API 密钥。
