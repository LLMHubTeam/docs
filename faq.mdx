...
<AccordionGroup>
  <Accordion title="我为什么要使用 OpenRouter？">
    ...
    OpenRouter 会将底层提供商的定价传递给用户，同时整合他们的正常运行时间，因此，您可以获得与直接从提供商处获得的相同的定价，同时拥有统一的 API 和回退机制，从而获得更好的正常运行时间。

  </Accordion>
  <Accordion title="如何开始使用 OpenRouter？">
    首先，请创建一个帐户并在
    [账户](https://www.llmhub.com.cn/console/topup)页面添加积分。积分只是您在 OpenRouter 上用于 LLM 推理的存款。当您使用 API 或聊天界面时，我们会从您的积分中扣除请求费用。每个模型和提供商每百万代币的价格都不同。
